...
import { IndustryInterface, SearchIndustryInterface } from './industry.response'
import { LoggerSystem } from '~/systems/logger'
import { IndustryModel } from './industry.model'

interface IIndustryService {
	findIndustries(): Promise<IndustryInterface>
	searchIndustries(queries: IndustryQuery): Promise<SearchIndustryInterface>
}
...
export default class IndustryService implements IIndustryService {
	...
	public findIndustries = async (): Promise<IndustryInterface> => {
		try {
	...
	public searchIndustries = async (
		...
		try {
			...
			const response: IndustryModel[] =
				await this._industryRepository.search(pagination, queries)
