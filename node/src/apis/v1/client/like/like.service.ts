...
interface ILikeService {
	findLikes(userid: number): Promise<LikeResponse>
	createLike(payload: CreateLikeDto, userid: number): Promise<BaseResponse>
	deleteLike(id: number): Promise<BaseResponse>
	checkExistLike(id: number): Promise<BaseResponse | undefined>
}
...
export default class LikeService implements ILikeService {
	...
	public findLikes = async (userid: number): Promise<LikeResponse> => {
		try {
			let total = 0
	...
	public deleteLike = async (id: number): Promise<BaseResponse> => {
		try {
