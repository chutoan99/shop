...
class LikeController {
	...
	public FindAll = async (
		...
		try {
			...
			const response = await this._likeService.findLikes(userid)
	...
	public Create = async (
		...
		try {
			...
			const response = await this._likeService.createLike(
				payload,
				req.user.userid
			)
			return res.status(STATUS_CODE.SUCCESSFUL.OK).json(response)
	...
	public Delete = async (
		...
		try {
			const response = await this._likeService.deleteLike(req.params.id)
			return res.status(STATUS_CODE.SUCCESSFUL.OK).json(response)
		} catch (error) {
			return internalServerError(res)
		}
