...
import { ModePopup, POPUP_DEFAULT_EXTRA } from '../../../resources'
import {
	customValidator,
...
export type ParamsFieldCheckbox = {
	TypeValue: IsValue
...
@Component({
	selector: 'app-field-config-checkbox',
	standalone: true,
	templateUrl: './field-config-checkbox.component.html',
	styleUrl: './field-config-checkbox.component.scss',
	imports: [
		CommonModule,
		DialogModule,
		ToastModule,
...
export class FieldConfigCheckBoxComponent extends FieldConfigBaseComponent {
	...
	private _createFormConfig(): FormGroup<IFormGroup<FieldDefault>> {
		return new FormGroup<IFormGroup<FieldDefault>>({
			...
			DataTypeSetting: new FormGroup<IFormGroup<ParamsFieldCheckbox>>({
				TypeValue: new FormControl(IsValue.ValueDefault),
				ValueDefault: new FormControl(false),
