...
import { AppConfig, BaseService, CryptoExtension, DataUtilsCore, PipeHostFilePipe } from 'channn-lib'
...
@Component({
	...
	styleUrl: './field-comment.component.scss',
	imports: [
		QuillModule,
		NgSwitch,
		NgClass,
...
export class FiledCommentComponent {
	@Input() dataDefine: string
	@Input() colDefine: any
	@Input() showBtnClose: boolean
	@Input() showBtnSend: boolean
	@Input() inputEvent!: () => void
	@Output() resultSubmit: EventEmitter<any> = new EventEmitter()
	@Output() resultCloseButton: EventEmitter<any> = new EventEmitter()
	...
	quillConfig = {
		...
		mention: {
			...
			renderItem: (item, searchTerm) => {
				return this.renderItemMention(item)
			},
			source: this.handleDateSelectMention.bind(this)
		}
	}
	constructor(
	...
	ngOnInit(): void {
		this.Comment = this.dataDefine['Comment']
		this.FileUploads = this.dataDefine['FileUploads']
	}
	renderItemMention(item) {
		let parent = document.createElement('p')
		let content = `
        <div class="cql-list-item-inner flex gap-2 py-2">
          <img src="${this.setSrcImage(item.colPic)}" class="max-w-9 w-9 h-9 rounded-full">
            <div class="content-center text-sm font-medium text-primary"> ${item.value}</div>
        </div>`
	...
	setSrcImage(src) {
		let resultSrc = ''
		let appNoImage = AppConfig.config.application.appNoImage
		let appImageUrl = AppConfig.config.apis.appImageUrl
		if (src == null || src == '') {
			resultSrc = appNoImage
		} else if (src == '/Content/GPG_CMS/assets/layouts/layout3/img/logo-gpg.png') {
			resultSrc = appNoImage
		} else if (src.startsWith('http')) {
			resultSrc = src
		} else {
			if (src.indexOf('/thumb') > -1) {
				resultSrc = appImageUrl + src.replace('/thumb', '')
			}
			resultSrc = appImageUrl + src
		}
		return resultSrc
	}
	deleteItemUpdate(index) {
	...
	onFileSelected(event) {
		...
		formData.append('tbname', this.colDefine.tbname)
		formData.append('Resize_Width', '300')
		formData.append('Resize_Height', '234')
		for (const file of Object.keys(files)) {
			formData.append(this.colDefine.colname, files[file])
			dataFile.push({
				File_Name: files[file]['name'],
				File_Size: files[file]['size'],
				File_Size_Name: DataUtilsCore.formatBytesFile(files[file]['size']),
				File_Type: files[file]['type'],
				File_Icon: DataUtilsCore.setIconFile(files[file]['type'])
			})
		}
		this._baseService.getFormAPIDefault('/api/data/uploadfiles', formData).subscribe(
			(resp) => {
				this._spinner.hide()
				if (resp.data.length == 0) {
					return
				}
				let dataSplit = resp.data[0][this.colDefine.colname].split(';')
				if (dataSplit.length == 0) {
					return
				}
				dataSplit.forEach((item, index) => {
					dataFile[index][this.colDefine.colname] = item.replace('/thumb/', '/')
					dataFile[index][this.colDefine.colname + '_Thumb'] = item
				})
				this.FileUploads = [...this.FileUploads, ...dataFile]
			},
			(err) => {
				this._spinner.hide()
	...
	handleEnter() {
		if (this.Comment === '' && this.FileUploads.length == 0) return
	...
	onClickClose($event) {
		this.Comment = ''
		this.FileUploads = []
		this.resultCloseButton.emit()
	}
	async handleDateSelectMention(searchTerm, renderList, mentionChar) {
		let [errSearch, respSearch] = await this._baseService.getDataAPI_Promise('GetDataInfo', {
			tbname: CryptoExtension.encryptTable(`vw_tbAccountInfo_User`),
			cols: CryptoExtension.encryptTable(`Employee_id as id,TenHT as value,colPic`),
			moreExp: CryptoExtension.encryptTable(`TenHT like N'%${searchTerm}%'`),
			isAction: '0',
			isNoConfig: '1'
		})
		renderList(respSearch.data)
	}
	onSelectionChangeDescription(e) {
		if (e.oldRange == null) {
			return
		}
		// this.resultSubmit.emit(this.htmlstring)
	}
	getMeEditorInstance(editorInstance: any, value): void {
		this.meQuillRef = editorInstance
		// this.meQuillRef.clipboard.dangerouslyPasteHTML(value);
	}
	async customImageCopy(dataUrl) {
		...
		let [errUpload, respUpload] = await this._baseService.getDataAPIDefault_Promise(
			...
			{
				tbname: CryptoExtension.encode(this.colDefine.tbname),
				colname: CryptoExtension.encode(this.colDefine.colname),
				dataUrl: dataUrl,
				Resize_Width: 300,
				Resize_Height: 234
			}
		...
		let dataFile = {
			...
			File_Icon: 'fa-solid fa-image',
			FileUploads: respUpload.data[0]['url'].replace('/thumb/', '/'),
			FileUploads_Thumb: respUpload.data[0]['url']
		}
		this.FileUploads = [...this.FileUploads, ...[dataFile]]
	}
		...
		input.onchange = async () => {
			...
			if (files) {
				...
				let [errUpload, respUpload] = await this._baseService.Upload_Promise({
					tbname: this.colDefine.tbname,
					colname: this.colDefine.colname,
					files: files
				})
				this._spinner.hide()
				if (errUpload) {
				...
				let dataSplit = respUpload.data[0][this.colDefine.colname].split(';')
				if (dataSplit.length == 0) {
					return
				}
				...
				dataSplit.forEach((row) => {
					...
					this.meQuillRef.insertEmbed(range.index, 'image', this.appImageUrl + row)
				})
			}
		}
	}
