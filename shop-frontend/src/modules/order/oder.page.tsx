...
import { CreateOrderDto, metadataOrderDto, variationOrder } from './interfaces'
import { useAppDispatch, useAppSelector } from '@hooks/hooks'
import { AppDispatch, RootState } from 'redux/app'
import { CartActions } from '@redux/cart.slice'
import { IUserModel } from '@models/user.model'
import { CartModel } from '@models/cart.model'
import { useCreateOrderMutation } from '@hooks/apis/order.hook'
...
function OderPage(): JSX.Element {
	...
	const dataUser: IUserModel = useAppSelector((state: RootState) => state.user)?.data
	const [loading, setLoading] = useState(false)
	const [total, setTotal] = useState(0)
	const [totalShip, setTotalShip] = useState(0)
	const [dataPayload, setDataPayload] = useState<CreateOrderDto>()
	const [createOrder] = useCreateOrderMutation()
	const { data: dataCart, isLoading, refetch } = useGetCartsQuery()
	...
	useEffect(() => {
		...
		const metadata: metadataOrderDto[] = data.reduce((result: any[], shop: CartModel[]) => {
			if (shop.length === 1) {
				const dataPost = dataTempt.find((data) => data.item_id === shop[0].item_id)?.overview
				if (dataPost) {
					const { variations, ...post } = dataPost
					const metadata = {
						shop_id: shop[0].shop_id,
						shop_name: shop[0].overview.shop_name,
						note: '',
						items: [
							{
								post,
								item_id: shop[0].item_id,
								amount: shop[0].amount,
								variation: shop[0].variation,
								price: shop[0].overview.price
							}
						]
					}
			...
			if (shop.length > 1) {
				...
				shop.forEach((cart) => {
					const dataPost = dataTempt.find((data) => data.item_id === cart.item_id)?.overview
					if (dataPost) {
						const { variations, ...post } = dataPost
						metadata.items.push({
							post,
		...
		const newData: CreateOrderDto = {
			ship_cost: ship,
			final_total: metadata.reduce((acc, curr) => {
				const shopTotal = curr.items.reduce((sum: number, item: any) => sum + item.price * item.amount, 0)
				return acc + shopTotal
			}, 0),
			total_num_items: metadata.reduce((acc, curr) => {
				const shopItemCount = curr.items.length || 0
				return acc + shopItemCount
			}, 0),
			user: {
				id: dataUser.id,
				...
				role: dataUser.role,
				name: dataUser.name,
				address_obj: dataUser.address_obj,
	...
	const updateNote = (newNote: string, index: number) => {
		if (!dataPayload || !dataPayload.metadata || !dataPayload.metadata[index]) {
			return
		}

		const updatedMetadata = [...dataPayload.metadata]
				...
				<div className='grid wide'>
					...
					<div className='bg-[#fff] mb-2.5 pt-7 pb-6 px-[30px]'>
						...
						<div>
							...
							<div className='font-bold text-[#222] inline pr-2.5'>
								{dataUser.phone ? `Mặc Định: ${[dataUser.address_obj?.address]} ` : ''}
							</div>
						</div>
					</div>
										...
										<div className='flex items-center justify-between pt-0 pb-3 px-0'>
											<div className='flex items-center whitespace-nowrap'>
												...
												<NavLink
													...
													to={`/shop/${ele[0]?.overview?.shop_id}`}>
													...
													<div className='min-w-0 flex flex-1 flex-col items-start pl-3 pr-0 py-0'>
														...
														<div className='mt-0 mx-0'>
															<div className='text-[rgba(0,0,0,0.54)] mb-[5px]'>
																Phân loại hàng:{' '}
																{item.variation?.map((v) => v.option).join(', ')}
															</div>
														</div>
													</div>
												</div>
