...
import { IShopMallModel } from '@models/shop-mall.model'
import { useGetShopMallQuery } from '@hooks/apis'
							...
							<div className='l-8'>
								<div className='l-12'>
									<Slider {...shopMallConfig.settingsSlider}>
										{data?.map((ele: IShopMallModel[], index: number) => (
											<div key={index} className='col col-3'>
												{ele?.map((item: IShopMallModel, index: number) => (
