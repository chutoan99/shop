...
import { ICategoryTreeModel } from '@models/category.model'
import { useGetCategoryTreeParentQuery } from '@hooks/apis/category.hook'

function FilterComponent(): JSX.Element {
	const params = useParams()
	const { data, isLoading } = useGetCategoryTreeParentQuery(Number(params.cat_id))
				...
				<div className='text-[rgba(0,0,0,0.8)]'>
					...
					<div>
						<div>
							...
							{!isLoading && (
								<>
									{data?.map((item: ICategoryTreeModel, index: number) => {
										return (
											<NavLink
