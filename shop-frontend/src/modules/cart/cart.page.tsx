...
import { buyCartActions } from '@redux/buy-cart.slice'
import { UpdateCartDto } from './dto'
import { CartModel, VariationCartModel } from '@models/cart.model'
import { VariationModel } from '@models/post-detail.model'
import { useDeleteCartMutation, useGetCartsQuery, useUpdateCartMutation } from '@hooks/apis/cart.hook'
...
function CartPage(): JSX.Element {
	...
	const dataCart = useGetCartsQuery()
	...
	const [agrsUpdate, setAgrsUpdate] = useState<{
		cartid: number
		currentVariation: VariationCartModel[]
		payload: {
			amount: number
			variation: VariationCartModel[]
		}
	...
	useEffect(() => {
		setAgrsUpdate((prev: any) => {
			return {
				...prev,
				currentVariation: selectedOption?.variation
			}
		})
	}, [selectedOption])

	useEffect(() => {
		dataCart.data?.response && setData(dataCart.data?.response)
	}, [dataCart])
	...
	const onAllCheck = () => {
		...
		if (!allChecked) {
			const itemIds = data?.flatMap((item: any) => item.map((e: CartModel) => e.id))
			setChecked(itemIds)
		} else {
			setChecked([])
		}
	...
	const onChangeOption = (item: CartModel, indexVariation: number, option: string) => {
		const { id, amount, variation } = item
		const updatedVariation = variation.map((v, i) => (i === indexVariation ? { ...v, option } : v))
		...
		setAgrsUpdate({
			...
			currentVariation: updatedVariation
		})
	}
	...
	const onShowOption = (indexItem: number, index: number, variation: VariationCartModel[]) => {
		setSelectedOption({ itemIndex: indexItem, variationIndex: index, variation })
	}
	...
	useEffect(() => {
		...
		const filteredData = dataTempt.filter((item) => checked.includes(item.id))
	...
	return (
		<>
			{dataCart.isLoading && <LoadingDefaultComponent />}
			{dataCart.data?.total === 0 && (
						...
						<div className='grid wide pt-[16px]'>
							<div className='sm-gutter'>
								<div className='w-full items-center pt-2.5 pb-[130px] '>
									...
									{data?.map((item: CartModel[], indexItem: number) => (
										<section
														...
														<div
															...
															<div className='flex items-center'>
																...
																<div className='w-[29.03811%] box-border relative flex-col flex'>
																	<div className='flex'>
																		<NavLink
																			to={`/product/${ele.item_id}/${ele.shop_id}`}>
																			<img
																																	...
																																	(
																																		...
																																		<button
																																			...
																																			className={`overflow-visible bg-[#fff] cursor-pointer min-w-[5rem] min-h-[2.125rem] box-border text-[rgba(0,0,0,0.8)] text-left border relative inline-flex items-center justify-center ml-0 mr-2 mt-0 mb-2 px-3 py-1 rounded-sm border-solid border-[rgba(0,0,0,0.09)] ${
																																				agrsUpdate?.currentVariation?.find(
																																					(
																																						o
																																					) =>
																																						o?.option ===
																	...
																	<div className='flex gap-[10px] items-center'>
																		{ele?.overview?.price_before_discount !== 0 && (
																			<span className='text-[rgba(0,0,0,0.54)] line-through'>
																				đ
																				{formatPrice(
																					ele?.overview?.price_before_discount
																				)}
																			</span>
																		)}
						...
						<div className='grid wide'>
							<section className="grid grid-cols-[1fr_20.3125rem_11.875rem] items-center box-border w-full flex-1 text-base relative mt-3 bg-[#fff] transition-[background] duration-[0.1s] ease-[cubic-bezier(0.4,0,0.6,1)] rounded-none before:content-[''] before:absolute before:top-[-1.25rem] before:h-5 before:w-full before:left-0">
								...
								<div className='col-start-1 col-end-4 flex items-center flex-1 w-full pl-5 pr-3 py-3'>
									...
									<button className='cursor-pointer'>Chọn Tất Cả ({dataCart.data?.total})</button>
