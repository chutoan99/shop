...
import { ICommandResponse } from '@core/interfaces'

export interface LoginResponse extends ICommandResponse {
...
export const useMutationRegister = (payload: RegisterDto) => {
	...
	const onRefetch = async () => {
		try {
			setLoading(true)
			const response: ICommandResponse = await ApiRegister(payload)
			if (response.err === 0) {
