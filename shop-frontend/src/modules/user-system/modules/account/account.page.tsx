...
import { FetchDistricts, FetchProvinces, FetchWards, UpdateUser, UploadImages } from './services'
import { DistrictModel, WardModel } from './interfaces'
import { useAppSelector } from '@hooks/hooks'
import { RootState } from 'redux/app'
import { LoadingCustomComponent, LoadingDefaultComponent } from '@modules/shared'
import { IUserModel } from '@models/user.model'
import { ProvinceModel } from './interfaces/province.model'
import { useGetUserCurrentQuery } from '@hooks/apis/user.hook'

export default function AccountPage(): JSX.Element {
	const { t } = useTranslation()
	const { refetch, isLoading: isLoadingUser } = useGetUserCurrentQuery()
	//? HANDLE ADDRESS
	const [payload, setPayload] = useState<IUserModel>(useAppSelector((state: RootState) => state.user).data)
	...
	const [provinceCode, setProvinceCode] = useState(payload?.address_obj?.provinceCode)
	const [districtCode, setDistrictCode] = useState(payload?.address_obj?.districtCode)
	const [wardCode, setWardCode] = useState(payload?.address_obj?.wardCode)
	const [numberHouse, setNumberHouse] = useState(payload?.address_obj?.address)
	...
	const onSubmit = () => {
		if (!payload?.address_obj) return toast.error(t(`USER_SYSTEM.ACCOUNT.VALIDATOR.INVALID_ADDRESS`))
	...
	const onUpdateUser = async (payload: IUserModel) => {
		try {
			setIsLoading(true)

			await Promise.resolve()
				.then(() => UploadImages(payload.avatar))
				.then((file) => {
					if (!file) {
