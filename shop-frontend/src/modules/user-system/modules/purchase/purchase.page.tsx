...
import { LIST_TAB, stateOrder } from './resources'
import { LoadingDefaultComponent } from '@modules/shared'
import { QueryOrderDto } from '@modules/order/interfaces'
import { OrderModel, ITabsModel } from '@models/order.model'
import { useSearchOrdersQuery } from '@hooks/apis/order.hook'
			...
			{isLoading ? (
				...
				<div className='col-lg-10'>
					<div className='w-full mb-[12px] flex overflow-hidden bg-[#fff]'>
						{LIST_TAB(data?.tab as ITabsModel)?.map((tab: any, index: number) => (
							<span
								key={index}
								onClick={() => {
					...
					{data?.response?.map((cart: OrderModel) => (
						<div className='shadow-[0_1px_1px_0_rgba(0,0,0,0.05)] mx-0 my-3 rounded-sm' key={cart.id}>
							<div className='px-6 py-3 bg-[#fff]'>
								{cart.metadata.map((meta, i: number) => (
									<>
										<div className='flex items-center justify-between px-0 pt-[10px]'>
											<div className='flex items-center whitespace-nowrap'>
												...
												<NavLink className='ml-2 mr-0 my-0' to={`/shop/${meta.shop_id}`}>
										...
										<hr className='border-b-[rgba(0,0,0,0.06)] border-b border-solid mt-[10px] ' />
										{meta.items.map((item) => (
											<Link to='#' key={item.post.id} className='mb-[10px]'>
												<div className=''>
													<div>
																...
																<div className=' w-[81px] h-[81px]'>
																	<NavLink
																		...
																		<div
																			...
																			style={{
																				backgroundImage: `${item.post?.image}`
																			}}>
																...
																<div>
																	<span className='align-middle text-sm leading-4 text-[rgba(0,0,0,0.87)]'>
																		<div
																			...
																			{item.post?.price_before_discount > 0 && (
