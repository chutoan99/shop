...
import { IProductDetailModel, VariationModel } from '@models/post-detail.model'
import { VariationCartModel } from '@models/cart.model'
import { useCreateCartMutation, useGetCartsQuery } from '@hooks/apis/cart.hook'
...
function ProductDetailComponent({ data }: ProductDetailProps): JSX.Element {
	...
	const initialPayload = useMemo<CreateCartDto>(
		() => ({
			item_id: +item_id! || 0,
			shop_id: +shop_id! || 0,
			amount: count,
		...
		[item_id, shop_id, count]
	)

	const [payload, setPayload] = useState<CreateCartDto>(initialPayload)

	const [payloadVariation, setPayloadVariation] = useState<VariationCartModel>({
		name: '',
		option: ''
	})
	...
	useEffect(() => {
		if (payloadVariation?.name) {
			setPayload((prev: CreateCartDto) => {
				const indexVariation = prev.variation.findIndex((v) => v.name?.trim() === payloadVariation.name.trim())
				if (indexVariation !== -1) {
					;(((prev.variation as VariationCartModel[])[indexVariation].name = payloadVariation.name),
						((prev.variation as VariationCartModel[])[indexVariation].option = payloadVariation.option))
				} else {
					;(prev.variation as VariationCartModel[]).push({
						name: payloadVariation.name,
						option: payloadVariation.option
					})
				}
				return {
					...prev,
	...
	}, [payloadVariation, setPayload])

	const onAddToCart = async () => {
		if (payload?.variation?.length === 0 || payload?.variation?.length !== data.variations.length) {
					...
					<div className='col c-12 mo-7 l-7 '>
						<div className='p-[10px]'>
							...
							<div
								...
								{data?.variations?.map((variation: VariationModel) => {
									return (
										<>
											...
											<div className='flex-wrap flex max-w-[32.1875rem]'>
												{variation?.options?.map((option: string, index: number) => (
													<button
														key={index}
														style={{
															outline: 0,
														...
														className={`overflow-visible bg-[#fff] cursor-pointer min-w-[5rem] min-h-[2.125rem] box-border text-[rgba(0,0,0,0.8)] text-left border relative inline-flex items-center justify-center ml-0 mr-2 mt-0 mb-2 px-3 py-1 rounded-sm border-solid border-[rgba(0,0,0,0.09)] ${
															payload.variation?.find((o) => o.option === option)
