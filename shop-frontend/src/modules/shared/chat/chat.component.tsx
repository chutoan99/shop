...
import ItemChatComponent from './components/item-chat.component'
...
import { useGetMessRoomQuery, useGetRoomsQuery } from '@hooks/apis/room.hook'

function ChatComponent(): JSX.Element {
	const { socketIo, isLoadingRoom, listMess, currentRoom, setListMess, setCurrentRoom } = useSocketIo()
	const user: IUserModel = useAppSelector((state: RootState) => state.user).data
	const [listRoom, setListRoom] = useState<IRoomExtra[] | null>([])
	...
	const {
		...
		refetch: refetchMessRoom
	} = useGetMessRoomQuery(listRoom && listRoom.length > 0 ? currentRoom?.roomId : 0)

	// Ref để cuộn xuống phần tử cuối cùng
	...
	useEffect(() => {
		if (!dataRooms) return
		dataRooms && setListRoom(dataRooms)
	}, [dataRooms])

	useEffect(() => {
		if (currentRoom?.roomId && listRoom?.length) {
			setIsListChat(true)
			refetchMessRoom()
			setListMess(dataMessRooms)
			socketIo.emit('join_room', currentRoom.roomId)
		}
	}, [currentRoom, listRoom, dataMessRooms, listMess])
	...
	const onAddMess = () => {
		if (message) {
			...
			socketIo.emit('send_message', payload)
													...
													<div className='flex flex-col h-full'>
														{listMess && listMess.length > 0 ? (
															<div className='grid grid-cols-12 gap-y-2'>
																{listMess.map((item: IMessageModel, index: number) => (
																	<div key={index}>
																		{item?.type === 'client' && (
																...
																{/* Phần tử cuối dùng để cuộn xuống */}
