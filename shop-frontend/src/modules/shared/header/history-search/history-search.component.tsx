...
import { HeaderCartComponent } from '../cart'
import { SuggestListComponent } from '../suggest-search'
import { HistorySearchModel } from '@models/history-search.model'
import { useGetHistorySearchQuery } from '@hooks/apis/history-search.hook'
import { useGetCartsQuery } from '@hooks/apis/cart.hook'
import { useGetSuggestSearchQuery } from '@hooks/apis/suggest-search.hook'

function HeaderHistorySearchComponent(): JSX.Element {
	...
	const { data: dataSuggestSearch, isLoading: isLoadingSuggestSearch } = useGetSuggestSearchQuery()
			...
			<div className='h-[82px] flex gap-[5px] items-center px-2 sm-gutter'>
				...
				<div className='w-full flex justify-center'>
					<div className='w-[90%]'>
										...
										<div className='absolute top-[calc(100%_-_-2px)] w-full bg-[#fff] shadow-[0_1px_5px_rgba(189,189,189)] overflow-hidden z-[3] rounded-[3px] left-0'>
											...
											{!isLoadingHistorySearch && (
												<div className='mt-1.5'>
													{dataHistorySearch?.map((item: HistorySearchModel) => (
														<div
															key={item.id}
						...
						{!isLoadingSuggestSearch && <SuggestListComponent data={dataSuggestSearch || []} />}
					</div>
				</div>
				<HeaderCartComponent
...
export default memo(HeaderHistorySearchComponent)
