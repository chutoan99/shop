CREATE DEFINER="avnadmin"@"%" PROCEDURE "sp_delete_crt_cart_by_postId"(
	IN postId BIGINT,
	IN userId BIGINT
)
BEGIN
    ...
    WHERE ca.item_id = postId AND ca.user_id = userId;
    ...
    -- Xóa giỏ hàng
    DELETE FROM defaultdb.carts ca WHERE ca.item_id = postId AND ca.user_id = userId;
